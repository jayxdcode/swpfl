...
(function() {
   ...
   */
        ...
        try {
            ...
            searchBtn.onclick = () => {
                ...
                if (manualQuery && manualQuery.trim() !== '') {
                    overlay.remove();
                    menu.remove();
                    const [title, artist] = currentTrackId.split('|');
            ...
            resetBtn.onclick = () => {
                try {
                    ...
                    window.debug("[RESET] trackKey to delete:", trackKey);
                    window.debug("[RESET] keys before delete:", Object.keys(config));
                    delete config[trackKey];
                    localStorage.setItem(CONFIG_KEY, JSON.stringify(config));
                    window.debug("[RESET] keys after delete:", Object.keys(config));
                    ...
                    loadLyrics(t, a, '', currentTrackDur, parsed => {
                        lyricsData = parsed;
                        renderLyrics(0);
                        if (currInf) setupProgressSync(currInf.bar, currInf.duration);
                    });
                } catch (error) {
                    window.debug("[RESET] Failed to reset pick:", error);
                }
            };
            ...
            useBtn.onclick = () => {
                ...
                (async function() {
                    ...
                    if (!lyricsConfig[trackKey].v2configured || confirm("Existing configs found. Proceed?")) {
                        ...
                        const { rom = "", transl = "" } = await fetchTranslations(rawLrc, inlineTr, t, a);
    ...
    function createPanel() {
        try {
            ...
            window.debug("[ERROR] createPanel error: ", e.message);
        }
    }
    ...
    function gmFetch(url, headers = {}, signal = null) {
        ...
        if (typeof GM_xmlhttpRequest === 'function') {
            ...
            return fetchViaBgWithSignal(url, { headers }, signal)
                .then(response => {
    ...
    function gmFetchPost(url, body = {}, headers = {}, signal = null) {
        ...
        if (typeof GM_xmlhttpRequest === 'function') {
            ...
            return fetchViaBgPost(url, { method: 'POST', headers, body: typeof body === 'string' ? body : JSON.stringify(body) }, signal)
                .then(response => {
                if (!response.ok) throw new Error(`HTTP error! status: ${response.status}`);
                return response;
            })
   ...
   */
    ...
    async function fetchTranslations(lrcText, humanTr, title, artist, signal = null) {
        try {
            const response = await gmFetchPost(BACKEND_URL, { lrcText, geniusLyrics: humanTr, title, artist }, { "Content-Type": "application/json" }, signal);
    ...
    async function loadLyrics(title, artist, album, duration, onTransReady, manual = { flag: false, query: "" }, signal = null) {
        ...
        try {
            ...
            */
            ...
            if (lyricsConfig[trackKey]?.manualLrc && !manual.flag) {
                ...
                onTransReady(parseLRC(addTimestamps(manualLrc), '', '').map(l => ({ ...l, time: l.time + offset })));
                const { rom, transl } = await fetchTranslations(manualLrc, geniusLyrics, title, artist, signal);
                onTransReady(parseLRC(addTimestamps(manualLrc), rom, transl).map(l => ({ ...l, time: l.time + offset })));
            ...
            const { rom, transl } = await fetchTranslations(rawLrc, geniusLyrics, title, artist, signal);
        ...
        try {
            ...
            if (info.id !== currentTrackId) {
                ...
                try {
                    ...
                    }, { flag: false, query: "" }, controller.signal);
                } catch (e) {
        ...
        const observerCallback = function(mutationsList) {
            ...
            if (mutationsList.some(m => m.type === 'childList')) {
                ...
                if (currentChildrenCount === 1) {
                    ...
                    (async function() { await sendLyricsContent(targetNode); })();
