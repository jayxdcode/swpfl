/*
 Element Picker — UMD build (v1.3.1) — DOMPurify-enabled
 ...
 - Uses DOMPurify (if available) to sanitize any template HTML assigned via innerHTML
 Version: 1.3.1 (DOMPurify-enabled)
*/
(function (root, factory) {
        ...
        // DOMPurify helper + safe sanitizer wrapper
        function sanitizeHTML(html){
                try {
                        if (typeof DOMPurify !== 'undefined' && DOMPurify && typeof DOMPurify.sanitize === 'function'){
                                return DOMPurify.sanitize(html);
                        } else {
                                // If DOMPurify isn't available, warn and return raw HTML to preserve structure.
                                // It's recommended to include DOMPurify in the page for security.
                                console.warn('DOMPurify not found — innerHTML assignments will use raw HTML. Include DOMPurify to sanitize content.');
                                return html;
                        }
                } catch (e){
        ...
        // small escape helper for inserting variable text safely when not using innerHTML
...
@media (max-width:480px){ #ep-details{ left:12px; right:12px; bottom:12px; padding:10px; } }`;
        ...
        function startElementPicker(opts = {}){
                ...
                details.innerHTML = sanitizeHTML(`
                ...
                function populateChildDropdown(parentEl, parentSelector){
                        ...
                        sel.innerHTML = sanitizeHTML('<option value="">-- show children --</option>');
                ...
                // Drag-and-drop sorter modal (now created with safe DOM APIs for variable data)
                function openSorter(storageKey, jsonKey, list){
                        ...
                        // Create header safely (use textContent for dynamic parts)
                        const h4 = document.createElement('h4');
                        h4.textContent = `Sort selectors — ${storageKey}.${jsonKey}`;
                        modal.appendChild(h4);

                        const listWrap = document.createElement('div');
                        listWrap.className = 'ep-sorter-list';
                        listWrap.id = 'ep-sorter-list';
                        modal.appendChild(listWrap);

                        const ctrlWrap = document.createElement('div');
                        ctrlWrap.style.display = 'flex';
                        ctrlWrap.style.gap = '8px';
                        ctrlWrap.style.justifyContent = 'flex-end';

                        const saveBtn = document.createElement('button');
                        saveBtn.id = 'ep-sort-save';
                        saveBtn.className = 'ep-mini';
                        saveBtn.textContent = 'Save';

                        const cancelBtn = document.createElement('button');
                        cancelBtn.id = 'ep-sort-cancel';
                        cancelBtn.className = 'ep-mini';
                        cancelBtn.textContent = 'Cancel';

                        ctrlWrap.appendChild(saveBtn);
                        ctrlWrap.appendChild(cancelBtn);
